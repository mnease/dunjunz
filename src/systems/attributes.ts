...
export function computePlayerDamage(save: SaveData): number {
  ...
  // Inline flag read (avoid circular import with fellowship.ts)
  const fellowAtk = save.flags?.['fellowship_dwarves'] ? 1 : 0;
  if (style === 'ranged') {
    const dex = eff.dex + imbueDex;
    const bonus = Math.floor(Math.max(0, dex - 1) / 2);
    return Math.max(1, 1 + weaponAtk + bonus + buff + fellowAtk);
  }
  if (style === 'magic') {
  ...
  return Math.max(1, 1 + weaponAtk + strBonus + buff + fellowAtk);
}
