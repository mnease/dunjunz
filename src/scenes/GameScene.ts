...
export class GameScene extends Phaser.Scene {
  ...
  /** Ambient animated map tiles (water/lava) — discrete tile render. */
  ...
  /**
   * Place discrete tile sprites for the room (pre-continuous / blocky restore).
   * One Phaser Image per logic cell — Core Keeper / classic Dunjunz path.
   * Also builds solid colliders. Soft entity outlines stay (textures.ts).
   */
  private placeRoomTiles(room: RoomDef, depth: number, weatherHere: boolean): void {
    this.ambientTiles = [];
      ...
      for (let x = 0; x < VIEW_TILES_W; x++) {
        ...
        const waterBody =
          kind === 'water'
            ...
            : undefined;
        // Pure resolve: structure → fluid → shore → themed land → land autotile → TEX
        // Themed beach/dwarvez keys are preferred over generic at-wall-*/at-floor-*.
        const candidates = resolveRoomCellTextureCandidates(
          this.tileGrid,
          x,
          y,
          {
        ...
        let texKey = pickTextureKey(candidates, (k) =>
          this.textures.exists(k),
        );

        // Fractal fill variants only when still on a plain legacy stamp (not at-*)
        const fluidKinds =
          kind === 'floor' ||
          kind === 'wall' ||
          kind === 'grass' ||
        ...
        const usedSpecial =
          texKey.startsWith('at-') ||
          texKey === 'tile-sand-wall' ||
          texKey === 'tile-dwarf-wall' ||
          texKey === 'tile-dwarf-floor' ||
          texKey === 'tile-sand' ||
          texKey.startsWith('tile-door') ||
          texKey.startsWith('tile-cave') ||
          texKey.startsWith('tile-stairs') ||
          texKey.startsWith('tile-locked') ||
          texKey.startsWith('tile-pad');
        if (
          !usedSpecial &&
          fluidKinds &&
        ...
        if (weatherHere) {
          ...
          if (kind === 'water' && this.seasonWeather.weather === 'icy') {
            ...
            kind === 'water' &&
            this.seasonWeather.weather === 'snowy'
          ) {
  ...
  private loadRoom(roomId: string, fromSave: boolean, entryFrom?: string): void {
    ...
    // Discrete tile sprites + solid colliders (blocky restore / pre-continuous)
    this.placeRoomTiles(room, depth, weatherHere);
  ...
  private unlockDoor(tx: number, ty: number): void {
    ...
    // Rebuild discrete tiles + solid colliders without full room reload
    this.walls.clear(true, true);
    this.children.list
      .filter((c) => (c as Phaser.GameObjects.Image).getData?.('mapTile'))
      .forEach((c) => c.destroy());
    this.ambientTiles = [];
    const depth = basementDepth(this.room.floor);
    const weatherHere = isWeatherRoom(this.room);
    this.placeRoomTiles(this.room, depth, weatherHere);
