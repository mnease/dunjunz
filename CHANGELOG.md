...
- **Full Terraria visual pass (EMA)** — characters/props/trees get outline + jagged silhouette + drop shadow; light cookies are stepped rings; rain/snow use hard pixel flakes; material borders nibble/jag. See `docs/terraria-visual-system-v1.md`.
...
- **Game version under play canvas** — lower-left of the stage shows `vX.Y.Z` from `package.json` (e.g. `v0.7.0`).
...
- **Beach wake animation** — crawler loads lying on the sand, sits up, then stands as blurry eyes clear and the voice greets them (movement locked until stand).
- **Beach dressing** — sand tiles + dune walls (not bricks), coconut palms, seaweed, scuttling non-combat crabs, and shorebound wave animation (foam rolls north toward the beach).
...
- **Lightning staff arc** — lightning staff casts a jagged electric bolt along facing (hits along the path, may chain-fork to a nearby foe) instead of a slow projectile.
...
- **Dialog covers the action** — talk box docks at the bottom of the screen instead of mid-screen so combat/exploration stays visible.
- **Beach load freeze** — outdoor shadow RenderTexture could be a dead GO after scene restart (`clear` → null `gl`). RTs are reset on create, validated before use, and rebuilt on WebGL errors. Crawler-id API failures no longer block wake.
...
- **World P2–P6 grammar** — asymmetric land budgets (Dunjunz long spine vs short Woodz/Dezertz surface ceremonies); quiet foyers + shrine sides; Woodz tree chokepoints / Dezertz cactus exposure; **Jaquays** b3↔b5 side loop + `b5_secret` / `sewerz_b3_secret`; ecology elites **Root Alpha**, **Dune Stalker**, **Bilge Brute** (mid-kill only, no land ceremony).
